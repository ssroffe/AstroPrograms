...
if (len(sys.argv) == 2) and sys.argv[1] is "lorentzian":
    ...
    def voigtModel(x, Gdepth, Gwidth):
        return 1.0-Gdepth*np.exp(-(x)**2/(2*Gwidth**2))

    def lnlikeModel(p,x,y,err):
        Gdepth,Gwidth = p
        return -np.sum((y-voigtModel(x,Gdepth,Gwidth))**2/(2*err))

    def lnpriorModel(p):
        Gdepth,Gwidth = p
        if 0.0 < Gdepth < 1.0 and 0.0 < Gwidth < 600.0:
            return 0.0
        return -np.inf
    ...
    def voigt(x, Gdepth, Gwidth, RVShift):
        return 1.0 - Gdepth*np.exp(-(x-RVShift)**2/(2*Gwidth**2))
    
    def lnlike(p,x,y,err):
        Gdepth,Gwidth,RVShift = p
        return -np.sum((y-voigt(x,Gdepth,Gwidth,RVShift))**2/(2*err))
...
for i in range(len(modelVels)):
    ...
    if i == 0:
        ...
        if (len(sys.argv) == 1) or (sys.argv[1] is "voigt"):
            global gd1
            gd1 = modelSamples[0].mean()
            global gw1
            gw1 = modelSamples[1].mean()
    elif i == 1:
        #global ld2
        ...
        if (len(sys.argv) == 1) or (sys.argv[1] is "voigt"):
            global gd2
            gd2 = modelSamples[0].mean()
            global gw2
            gw2 = modelSamples[1].mean()
    elif i == 2:
        #global ld3
        ...
        if (len(sys.argv) == 1) or (sys.argv[1] is "voigt"):
            global gd3
            gd3 = modelSamples[0].mean()
            global gw3
            gw3 = modelSamples[1].mean() 
...
#for j in range(0,1):
    ...
    for i in range(len(vels)):
        ...
        plt.plot(vels[i],voigt(vels[i],gd,gw,rvFit)+i*off,color='k',linewidth=1.5)
        plt.plot(vels[i],voigt(vels[i],gd,gw,rvFit+rvStd)+i*off,color='c',linewidth=1.5)
        plt.plot(vels[i],voigt(vels[i],gd,gw,rvFit-rvStd)+i*off,color='c',linewidth=1.5)
