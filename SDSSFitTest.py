...
#for j in range(0,1):
    ...
    sdssSampler1 = tls.MCMCfit(lnprob1,args=(np.array(sdssVels[0]),np.array(sdssFluxes[0]),np.array(sdssErrs[0])),nwalkers=nwalkers,ndim=ndim,burnInSteps=8000,steps=8000)
    sdssSampler2 = tls.MCMCfit(lnprob2,args=(np.array(sdssVels[1]),np.array(sdssFluxes[1]),np.array(sdssErrs[1])),nwalkers=nwalkers,ndim=ndim,burnInSteps=8000,steps=8000)
    sdssSampler3 = tls.MCMCfit(lnprob3,args=(np.array(sdssVels[2]),np.array(sdssFluxes[2]),np.array(sdssErrs[2])),nwalkers=nwalkers,ndim=ndim,burnInSteps=8000,steps=8000)

    sdssRV1, sdssSTD1 = tls.GetRV(sdssSampler1)
    sdssRV2, sdssSTD2 = tls.GetRV(sdssSampler2)
    sdssRV3, sdssSTD3 = tls.GetRV(sdssSampler3)

    sdssSamplesChain = sdssSampler.chain[:,:,:].reshape((-1,ndim))
    sdssSamplesChain1 = sdssSampler1.chain[:,:,:].reshape((-1,ndim))
    sdssSamplesChain2 = sdssSampler2.chain[:,:,:].reshape((-1,ndim))
    sdssSamplesChain3 = sdssSampler3.chain[:,:,:].reshape((-1,ndim))
    ...
    print "\n"
    print sdssRV1,sdssSTD1
    print sdssRV2,sdssSTD2
    print sdssRV3,sdssSTD3
    print "\n"
    plot_format()
    for i in range(len(modelVels)):
    ...
    plt.plot(sdssVels[0],voigt(sdssVels[0],ld1,lw1,gd1,gw1,sdssRV1)+0*off,color='k',linewidth=1.5,label='data fits')
    ...
    plt.plot(sdssVels[1],voigt(sdssVels[1],ld2,lw2,gd2,gw2,sdssRV2)+1*off,color='k',linewidth=1.5)
    #plt.plot(sdssVels[1],voigt(sdssVels[1],ld2,lw2,gd2,gw2,sdssRV+sdssSTD)+1*off,color='purple',linewidth=1.5,ls='--')
    #plt.plot(sdssVels[1],voigt(sdssVels[1],ld2,lw2,gd2,gw2,sdssRV-sdssSTD)+1*off,color='purple',linewidth=1.5,ls='--')
    

    
    plt.plot(sdssVels[2],voigt(sdssVels[2],ld3,lw3,gd3,gw3,sdssRV3)+2*off,color='k',linewidth=1.5)
    ...
    sdssVelFig1 = corner.corner(sdssSamplesChain1,label=["RV"])
    sdssVelFig1.savefig("sdssFits/corner_sdssVelFits1.pdf")
    sdssVelFig2 = corner.corner(sdssSamplesChain2,label=["RV"])
    sdssVelFig2.savefig("sdssFits/corner_sdssVelFits2.pdf")
    sdssVelFig3 = corner.corner(sdssSamplesChain3,label=["RV"])
    sdssVelFig3.savefig("sdssFits/corner_sdssVelFits3.pdf")
