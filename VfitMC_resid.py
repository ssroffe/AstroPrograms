...
for j in range(0,1):
    ...
    residual = flux - lorentzian(vel,depthFit,widthFit,RVFit)
    ...
    plt.step(vel,flux,where='mid',linewidth=2,label=lineNames[lineIndex])
    #plt.errorbar(vel,flux,linewidth=2,yerr=ferr,label=lineNames[lineIndex],ls='None')
    plt.plot(vel,lorentzian(vel,depthFit,widthFit,RVFit),color='black',linewidth=2)
    ...
    plt.plot(vel,residual)

    plt.savefig("fitPlots/"+wdName+"_mc_fit.pdf")
