...
for j in range(0,1):
    ...
    burnInSamples = sampler.chain[:,:,:].reshape((-1,ndim))

    tls.plotchains(sampler,7,"tmp/burnIn_"+wdName,".pdf")
    sampler.reset()
    ...
    tls.plotchains(sampler,7,"tmp/"+wdName,".pdf")

    d1FitArr = samplesTrans[0]
    w1FitArr = samplesTrans[1]
    d2FitArr = samplesTrans[2]
    w2FitArr = samplesTrans[3]
    d3FitArr = samplesTrans[4]
    w3FitArr = samplesTrans[5]
    RVFitArr = samplesTrans[6]
    ...
    d1MCMC,w1MCMC,d2MCMC,w2MCMC,d3MCMC,w3MCMC,RVMCMC = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]),zip(*np.percentile(samples, [16, 50, 86],axis=0)))
...
"""
    ...
    plt.step(vels[0],fluxes[0]+0*off,where='mid',linewidth=2,label=lineNames[1])
    plt.step(vels[1],fluxes[1]+1*off,where='mid',linewidth=2,label=lineNames[2])
    plt.step(vels[2],fluxes[2]+2*off,where='mid',linewidth=2,label=lineNames[3])
